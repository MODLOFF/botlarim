...
if(!member || !isim || !yaş ) return message.channel.send(embed.setDescription(`\`Eksik Argüman Kullandınız. Örnek: ${modloff.prefix}erkek @MODLOFF\ID\``))
await member.roles.set([modloff.erkekrol]).catch(e => { });
...
if(!UyeData){ let newUyeData = new RegisterData({ guildId: message.guild.id, userId: member.id, isimgecmisi:  [{ isim: `${modloff.tag} ${isim.charAt(0).toUpperCase() + isim.slice(1).toLowerCase()}${yaş ? ` ' ${yaş}` : ``}`, type: `Erkek`}] }).save(); }
