...
const ayarlar = require('../../setup/guard2.json');
const fs = require('fs');

client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "MODLOFF ❤️ MODLOFF" }, status: "idle" });
...
client.on("message", async message => {
  ...
  let embed = new MessageEmbed().setColor("#00ffdd").setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, })).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "MODLOFF"} is coming...!`).setTimestamp();
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "MODLOFF Koruma" }).catch();
};
...
client.on("channelDelete", async channel => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.channelGuard) return;
  cezalandir(entry.executor.id, "ban");
  await channel.clone({ reason: "MODLOFF Kanal Koruma" }).then(async kanal => {
